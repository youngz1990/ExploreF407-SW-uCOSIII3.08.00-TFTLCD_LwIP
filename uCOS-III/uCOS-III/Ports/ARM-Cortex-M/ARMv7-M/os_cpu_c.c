...
void  OS_CPU_SysTickInit (CPU_INT32U  cnts)
{
    ...
    //这样函数始终可以将systick中断设置为kernal aware中断中优先级最高的中断
                                                                /* Set BASEPRI boundary from the configuration.         */
    basepri             = (CPU_INT32U)(CPU_CFG_KA_IPL_BOUNDARY << (8u - CPU_CFG_NVIC_PRIO_BITS));
    CPU_REG_SYST_RVR    = cnts - 1u;                            /* Set Reload Register                                  */

    //Cortex-M3/M4支持256个中断优先级(8为)，因此用于表示每个异常优先级的寄存器被设计成8位
    //(猜测是采用硬件的方式来比较每个异常的优先级，如果表示优先级的位越多硬件越复杂，因此折中选择8位)
    //对这些寄存器的读写采用效率最高的32位方式，因此将每4个寄存器分为1组
